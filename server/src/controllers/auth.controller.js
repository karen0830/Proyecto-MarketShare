...
export const imageProfile = async (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    const storagePath =
      "images/" + archivo[0].newFilename + archivo[0].originalFilename; // Ruta en Firebase Storage donde se guardará el archivo
...
export const updateProfilePublications = async (req, res, url) => {
  try {
    ...
    console.log(
      "ProfileImage actualizada en todas las publicaciones del usuario"
    );
  } catch (error) {
...
export const addPublications = async (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    console.log(archivo[0]);
    const storagePath =
      "publications/" + archivo[0].newFilename + archivo[0].originalFilename; // Ruta en Firebase Storage donde se guardará el archivo
    const file = bucket.file(storagePath);
    const localReadStream = fs.createReadStream(archivo[0]._writeStream.path);
    const stream = file.createWriteStream({
      metadata: {
...
export const addPublicationsVideo = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    const originalFileName = files.publication
      ? files.publication[0].originalFilename
      : null;
...
export const getAllPublications = async (req, res) => {
  ...
  publications = await User.find(
    { username: { $ne: decodedToken.name } },
    "publications"
  );
...
export const postMessage = async (req, res) => {
  ...
  try {
    ...
    for (const element of messages) {
      ...
      if (
        (chatUsersElement.userOne === from &&
          chatUsersElement.userTwo === username) ||
        (chatUsersElement.userOne === username &&
          chatUsersElement.userTwo === from)
      ) {
        chatMessage = true;
        ...
        await User.findOneAndUpdate(
          {
            username: from,
            "messages.ChatUsers.userOne": chatUsersElement.userOne,
            "messages.ChatUsers.userTwo": chatUsersElement.userTwo,
          },
          {
            $set: {
...
export const getMessage = async (req, res) => {
  ...
  try {
    ...
    for (const element of messages) {
      ...
      // Verifica si ya existe una conversación entre 'from' y 'to'
      if (
        (chatUsersElement.userOne === user &&
          chatUsersElement.userTwo === username) ||
        (chatUsersElement.userOne === username &&
          chatUsersElement.userTwo === user)
      ) {
        chatMessage = true;
