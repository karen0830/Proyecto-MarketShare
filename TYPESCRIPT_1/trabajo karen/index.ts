...
let trues: boolean = true;
let falses: boolean = false;
let numero1: number = 23234;
...
}else console.log(`Bienvenido ${nombres} ${apellidos} a nuestro sitio web`);
...
const funcionParametros = function(boolens:boolean, strings?:string, numero:number = 3455){
    boolens = numero >= 18;
    let mayor:string = "";
    if(boolens){
        mayor = "mayor de edad"
    }else mayor = "menor de edad"
...
const suma = function(numero:number, numero1:number){
...
// reservada return.
const flecha = (numero1: number, numero2: number) => console.log(`producto de ${numero1} y ${numero2} es: ${numero1 * numero2}`);
flecha(2, 1);
...
const fecha = (numero1: number, numero2: number) => {
    let modulo: number = numero1 % numero2;
...
//2
const estudiante: { nombre: string, edad: number, carrera: string } = {
    nombre: 'María',
    edad: 21,
    carrera: 'Ingeniería'
};
